<?php
/**
 * FC theme — editable CSS custom properties (saved to data/theme.json).
 */
 ...
 */
function fc_theme_defaults(): array
{
    return [
 ...
 */
function fc_theme_presets(): array
{
    $neutral = [
        '--fc-green' => '#04a725',
        '--fc-texas-green' => '#0b9321',
 ...
 * @return string|null
 */
function fc_theme_detect_preset(array $colors): ?string
{
    $defaults = fc_theme_defaults();
    foreach (fc_theme_presets() as $id => $preset) {
        $match = true;
 ...
 */
function fc_theme_schema(): array
{
    return [
        'brand' => [
            'label' => 'Brand & accent',
...
function fc_theme_file_path(): string
{
    return dirname(__DIR__) . DIRECTORY_SEPARATOR . 'data' . DIRECTORY_SEPARATOR . 'theme.json';
}

/**
 * @return array<string, mixed>
 */
function fc_theme_read_file(): array
{
...
function fc_theme_get(): array
{
    ...
    foreach ($saved as $var => $value) {
        ...
        $normalized = fc_theme_normalize_color($value);
        if ($normalized !== null) {
...
function fc_theme_normalize_color(string $value): ?string
{
    $value = trim($value);
    if ($value === '') {
        return null;
 ...
 */
function fc_theme_save(array $colors): array
{
    $defaults = fc_theme_defaults();
    $next = $defaults;
    ...
    foreach ($colors as $var => $value) {
        ...
        $normalized = fc_theme_normalize_color((string) $value);
        if ($normalized === null) {
            return [
                'ok' => false,
 ...
 */
function fc_theme_hex_to_rgb(string $hex): ?array
{
    $hex = fc_theme_normalize_color($hex);
    if ($hex === null) {
        return null;
    }
...
function fc_theme_rgba(array $rgb, float $alpha): string
{
 ...
 * @return array<string, string>
 */
function fc_theme_expand_colors(array $colors): array
{
    $expanded = $colors;
    $accent = $colors['--fc-princeton-orange'] ?? '#f67925';
    $rgb = fc_theme_hex_to_rgb($accent);

    if ($rgb !== null) {
        $expanded['--fc-a-orange-3'] = fc_theme_rgba($rgb, 0.031);
        $expanded['--fc-a-orange-20'] = fc_theme_rgba($rgb, 0.2);
        $expanded['--fc-a-orange-22'] = fc_theme_rgba($rgb, 0.22);
        $expanded['--fc-a-orange-25'] = fc_theme_rgba($rgb, 0.25);
        $expanded['--fc-a-orange-35'] = fc_theme_rgba($rgb, 0.35);
        $expanded['--fc-a-orange-38'] = fc_theme_rgba($rgb, 0.38);
    }

    return $expanded;
}

function fc_theme_css_block(): string
{
    $colors = fc_theme_expand_colors(fc_theme_get());
    $lines = [];
    foreach ($colors as $var => $value) {
        $lines[] = '    ' . $var . ': ' . $value . ';';
    }
 ...
 */
function fc_theme_api_payload(): array
{
    $file = fc_theme_read_file();
    ...
    return [
        ...
        'colors' => fc_theme_get(),
        'defaults' => fc_theme_defaults(),
        'schema' => fc_theme_schema(),
        'presets' => array_values(fc_theme_presets()),
        'activePreset' => fc_theme_detect_preset(fc_theme_get()),
