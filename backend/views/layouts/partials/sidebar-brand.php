...
$fcAdminSites = function_exists('fc_admin_sites_list') ? fc_admin_sites_list() : [];
$fcAdminCurrentSite = function_exists('fc_admin_current_site') ? fc_admin_current_site() : null;
$fcAdminHomeSite = function_exists('fc_admin_home_site') ? fc_admin_home_site() : null;
$fcSiteHomeKey = is_array($fcAdminHomeSite) ? (string) ($fcAdminHomeSite['key'] ?? '') : '';
$fcSiteSwitchToken = function_exists('fc_auth_csrf_token') ? fc_auth_csrf_token() : '';
...
$fcSiteCurrentLogoUrl = ($fcSiteCurrentLogo !== '' && isset($fcAppBase))
    ...
    : '';
$fcBrandingLogoUrl = (isset($fcAppBase) && function_exists('fc_branding_logo_url'))
    ? fc_branding_logo_url((string) $fcAppBase, is_array($fcBranding ?? null) ? $fcBranding : null)
    : '';
