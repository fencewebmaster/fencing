 ...
 *
 * Read-only template: FenceStylePresenter::listViewData() guarantees every shape
 * here. Escaping via the global e() helper; the layout's is_array() check is the gate.
 *
...
<div class="fc-fs-styles-page" data-fc-fence-styles-server="1">
    ...
    <div class="fc-admin-fence-styles">
        <div class="fc-admin-fence-styles__grid">
            ...
            <a
                href="<?php echo e((string) ($card['edit_href'] ?? '#')); ?>"
                class="fencing-style-item fc-admin-fence-style-item fc-admin-fence-style-link"
                data-route="<?php echo e((string) ($card['edit_route'] ?? '')); ?>"
                data-title="<?php echo e((!empty($card['can_edit']) ? 'Edit ' : 'View ') . ($card['title'] ?? '')); ?>"
                aria-label="<?php echo e((!empty($card['can_edit']) ? 'Edit ' : 'View ') . ($card['title'] ?? '')); ?>"
            >
            <?php else : ?>
            <div
                class="fencing-style-item fc-admin-fence-style-item"
                aria-label="<?php echo e((string) ($card['title'] ?? '')); ?>"
            >
            <?php endif; ?>
                <div>
                    <div class="fencing-style-img">
                        <?php if (!empty($card['has_image'])) : ?>
                        <img src="<?php echo e((string) ($card['image_url'] ?? '')); ?>" alt="" loading="lazy" decoding="async">
                        <?php else : ?>
            ...
            <div class="fc-fs-card-controls">
                <label class="fc-fs-card-check" title="<?php echo e('Select ' . (string) ($card['title'] ?? '') . ' (Ctrl+Click a style to select it)'); ?>">
                    <input
                        type="checkbox"
                        class="fc-fs-card-check-input"
                        data-fc-fs-card-select
                        data-slug="<?php echo e((string) ($card['slug'] ?? '')); ?>"
                        aria-label="<?php echo e('Select ' . (string) ($card['title'] ?? '')); ?>"
                    >
