...
final class NotFoundHandler
{
    ...
    public static function plannerUrl(): string
    {
        ...
        if (str_ends_with($dir, '/public') || str_ends_with($dir, '/backend')) {
    ...
    public static function detectArea(): string
    {
        ...
        if (str_contains($script, '/public/') || str_ends_with($script, '/public')) {
            return 'admin';
        }

        $uri = str_replace('\\', '/', (string) (parse_url((string) ($_SERVER['REQUEST_URI'] ?? ''), PHP_URL_PATH) ?? ''));
        if (str_contains($uri, '/public/') || str_contains($uri, '/backend/') || str_ends_with($uri, '/backend')) {
            return 'admin';
        }
     ...
     */
    public static function abort(string $area = 'auto', string $message = ''): void
    {
        if ($area === 'auto') {
            $area = self::detectArea();
        }
        if ($area !== 'admin') {
            $area = 'frontend';
        }
        ...
        if ($area === 'admin') {
            $homeUrl = AuthService::dashboardUrl();
            $homeLabel = 'Back to Dashboard';
            $defaultMessage = 'This admin page could not be found.';
        } else {
            $homeUrl = self::plannerUrl();
